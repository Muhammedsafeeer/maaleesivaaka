 ...
 * storage) so either client can edit the layout and the other picks up the change.
...
export function PosterSettingsDesigner({ settings }: { settings: PosterSettings }) {
  ...
  const [fields, setFields] = useState<PosterField[]>(settings.fields);
  ...
  async function handleBackgroundChange(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    const persistResult = await updatePosterBackgroundAction(uploadResult.data);
    setUploading(false);
  ...
  function handleSave() {
    startSaveTransition(async () => {
      const result = await updatePosterFieldsAction(fields);
      if (result.error) {
    ...
    return (
      <div className="flex flex-col items-center gap-3 rounded-lg border border-dashed border-border p-12 text-center">
        ...
        <p className="text-sm text-muted-foreground">
          Upload a background image to start designing the poster.
        </p>
        <label>
          <input
            type="file"
